...
void PBKDF2::sha256Transform(uint32_t state[8], const unsigned char block[64]) {
    uint32_t w[64];
...
void PBKDF2::sha256(const unsigned char* data, size_t len, unsigned char* hash) {
    uint32_t state[8] = {
...
void PBKDF2::hmacSha256(const unsigned char* key, size_t keyLen,
    ...
    unsigned char inner[64 + dataLen];
...
std::vector<unsigned char> PBKDF2::deriveKey(const unsigned char* password, size_t passwordLen,
    ...
    std::vector<unsigned char> derivedKey(keyLen);
    const size_t hLen = 32; // SHA-256 output length
    size_t numBlocks = (keyLen + hLen - 1) / hLen;
    
    for (size_t block = 1; block <= numBlocks; block++) {
        unsigned char saltBlock[saltLen + 4];
        ...
        size_t toCopy = (offset + hLen <= keyLen) ? hLen : (keyLen - offset);
        memcpy(&derivedKey[offset], T, toCopy);
    }
    
    return derivedKey;
}

std::vector<unsigned char> PBKDF2::deriveKey(const std::string& password,
                                             const std::vector<unsigned char>& salt,
                                             unsigned int iterations, size_t keyLen) {
    return deriveKey(reinterpret_cast<const unsigned char*>(password.c_str()),
                    password.length(), salt.data(), salt.size(), iterations, keyLen);
}

std::vector<unsigned char> PBKDF2::generateSalt(size_t length) {
